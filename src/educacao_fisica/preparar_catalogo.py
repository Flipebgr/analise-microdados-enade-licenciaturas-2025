...
def aplicar_recorte_educacao_fisica(cursos: pd.DataFrame) -> pd.DataFrame:
    """Cria recortes exclusivos sem inventar um grupo UFPA Conceito 1."""
    ...
    municipios = out["MUNICIPIO"].astype("string")
    if "CO_MUNIC_CURSO" in out.columns:
        municipios = municipios.fillna(
            out["CO_MUNIC_CURSO"].astype("string")
        )
    recorte_ufpa = "UFPA — " + municipios

    out["RECORTE_EDUCACAO_FISICA"] = pd.Series(
        np.select(
            [
                ies.eq(EDUCACAO_FISICA.co_ies_focal),
                ~ies.eq(EDUCACAO_FISICA.co_ies_focal) & uf.eq(15),
...
def preparar_catalogo_educacao_fisica(
    extraida: Path,
    conceito_path: Path,
    ...
    cadastro = cadastro[
        cadastro["CO_GRUPO"].eq(EDUCACAO_FISICA.co_grupo)
    ...
    conceitos = conceitos[
        conceitos["CO_GRUPO"].eq(EDUCACAO_FISICA.co_grupo)
    ].copy()
    ...
    validar_unicidade_por_curso(
        conceitos,
        nome="conceitos de Educação Física",
    )
    ...
    cursos = aplicar_grupos_area(cursos, EDUCACAO_FISICA)
    cursos = aplicar_recorte_educacao_fisica(cursos)
    validar_grupos(
        cursos,
...
def construir_auditoria_fontes_ufpa(
    ...
    cad = cadastro.loc[
        cadastro["CO_IES"].eq(EDUCACAO_FISICA.co_ies_focal),
    ...
    audit["NO_CADASTRO_MICRODADOS"] = (
        audit["NO_CADASTRO_MICRODADOS"].fillna(False)
    )
    audit["NA_PLANILHA_CONCEITO"] = (
        audit["NA_PLANILHA_CONCEITO"].fillna(False)
    )
...
def tabela_mestra_ufpa(cursos: pd.DataFrame) -> pd.DataFrame:
    ...
    ufpa = cursos[
        cursos["CO_IES"].eq(EDUCACAO_FISICA.co_ies_focal)
    ].copy()
