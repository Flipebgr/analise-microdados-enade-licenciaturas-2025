...
from src.educacao_fisica.agregar_educacao_fisica import (
    agregar_temas_educacao_fisica,
)
...
from src.educacao_fisica.preparar_catalogo import (
    preparar_catalogo_educacao_fisica,
    tabela_mestra_ufpa,
)
from src.educacao_fisica.validar_educacao_fisica import (
    validar_base_educacao_fisica,
)
...
def main() -> int:
    ...
    logger = configurar_logger(
        ROOT / "logs" / "educacao_fisica_base.log"
    )

    extraida = caminho_relativo(cfg["arquivos"]["pasta_extraida"])
    conceito_path = caminho_relativo(
        cfg["arquivos"]["conceito_enade"]
    )
    encoding_saida = cfg["saida"]["csv_encoding"]

    if not conceito_path.exists():
        logger.error(
            "Planilha de conceito ausente: %s",
            conceito_path,
        )
        return 2

    try:
        arq1 = encontrar_arquivo(
            extraida,
            "microdados2025_arq1.txt",
        )
    ...
    logger.info(
        "Preparando catálogo nacional de %s",
        EDUCACAO_FISICA.nome,
    )
    cursos, auditoria = preparar_catalogo_educacao_fisica(
        extraida,
        conceito_path,
    )
    ...
    logger.info(
        "Agregando desempenho, perfil, trajetória e questionários"
    )
    temas = agregar_temas_educacao_fisica(
        pasta_dados,
        ids_cursos,
    )
    ...
    benchmarks_amplos = resumo_por_grupo(
        base,
        indicadores_benchmark,
    )

    out = ROOT / "dados_processados" / "educacao_fisica"
    produtos = {
        "cursos_educacao_fisica.csv": cursos,
        ...
        "desempenho_individual_mesmo_arquivo.csv": (
        ...
        "agregado_processo_formativo.csv": (
            temas["processo_formativo"]
        ),
        ...
        "comparacao_ufpa_campi.csv": comparacao_interna,
        "distribuicao_sexo.csv": temas["distribuicao_sexo"],
        "distribuicao_turno.csv": temas["distribuicao_turno"],
        "distribuicao_socioeconomica.csv": (
            temas["distribuicao_socioeconomica"]
        ),
        "regras_indicadores_socioeconomicos.csv": (
            temas["regras_socioeconomicos"]
        ),
        "itens_processo_formativo.csv": (
            temas["itens_processo_formativo"]
        ),
        "diagnostico_consistencia_processo.csv": (
            temas["diagnostico_processo"]
        ),
        "distribuicao_recomendacao.csv": (
            temas["distribuicao_recomendacao"]
        ),
    }
    ...
    print("\nEducação Física — UFPA")
    print(ufpa.to_string(index=False))
    print(
        "\nBase concluída: "
        f"{len(base)} cursos nacionais; "
        f"{len(ufpa)} ofertas UFPA."
    )

    logger.info(
        "Base concluída: %s cursos nacionais; %s ofertas UFPA",
        len(base),
        len(ufpa),
    )
    return 0
