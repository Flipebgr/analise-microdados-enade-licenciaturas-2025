...
def aplicar_recorte_pedagogia(cursos: pd.DataFrame) -> pd.DataFrame:
    """Cria recortes exclusivos adequados a Pedagogia, sem inventar Grupo A."""
    out = cursos.copy()
    ...
    out["REFERENCIA_INTERNA_CASTANHAL"] = curso.eq(CO_CURSO_CASTANHAL)
    out["RECORTE_PEDAGOGIA"] = pd.Series(
        np.select(
            [
                ies.eq(PEDAGOGIA.co_ies_focal) & conceito.eq(5),
                ies.eq(PEDAGOGIA.co_ies_focal) & conceito.eq(4),
                ~ies.eq(PEDAGOGIA.co_ies_focal) & uf.eq(15),
...
def preparar_catalogo_pedagogia(extraida: Path, conceito_path: Path) -> pd.DataFrame:
    ...
    cadastro = cadastro[cadastro["CO_GRUPO"].eq(PEDAGOGIA.co_grupo)].drop_duplicates("CO_CURSO")
    adicionais = tuple(
        coluna for coluna in COLUNAS_CADASTRO
        if coluna not in {"CO_CURSO", "CO_IES", "CO_GRUPO"}
    )
    cadastro = preparar_catalogo_area(
        cadastro, PEDAGOGIA, colunas_adicionais=adicionais
    ).drop(columns=["IES_FOCAL", "AREA_SLUG", "AREA_NOME"])

    conceitos = carregar_conceitos(conceito_path)
    conceitos = conceitos[conceitos["CO_GRUPO"].eq(PEDAGOGIA.co_grupo)].copy()
    ...
    validar_unicidade_por_curso(conceitos, nome="conceitos de Pedagogia")
    ...
    cursos = aplicar_grupos_area(cursos, PEDAGOGIA)
    cursos = aplicar_recorte_pedagogia(cursos)
