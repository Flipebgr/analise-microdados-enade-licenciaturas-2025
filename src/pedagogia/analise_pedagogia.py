...
def construir_comparacao_recortes(
    ...
    for recorte in RECORTES_PEDAGOGIA:
        sub = base[base["RECORTE_PEDAGOGIA"].eq(recorte)]
...
def construir_benchmarks_por_oferta(
    ...
    """Seleciona, para cada oferta UFPA, cursos externos estruturalmente comparáveis."""
    ufpa = base[base["CO_IES"].eq(PEDAGOGIA.co_ies_focal)].copy()
    externos = base[~base["CO_IES"].eq(PEDAGOGIA.co_ies_focal)].copy()
    ...
    for _, alvo in ufpa.iterrows():
        ...
        for indicador in ("nt_ger_mean", "nt_obj_mean", "nt_dis_mean"):
            valores = pd.to_numeric(selecionados.get(indicador), errors="coerce").dropna()
