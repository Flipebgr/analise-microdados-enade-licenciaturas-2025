...
def participacao_ufpa_validada(base: pd.DataFrame, path: Path) -> None:
    ...
    ax.bar(
        ...
        oficial,
        largura,
        label="Participação oficial",
    )
    ax.bar(
        x + largura / 2,
        micro,
        largura,
        label="Presença nos microdados",
    )
    ax.set_xticks(x, ufpa["ROTULO_OFERTA"], rotation=25, ha="right")
    ax.set_ylabel("Percentual (%)")
    ax.set_title("Geografia — auditoria de participação das ofertas da UFPA")
    ax.legend()
    _salvar(fig, path)


def contraste_interno_validado(
    ...
    df = contraste.loc[
        contraste["INDICADOR"].isin(indicadores)
    ].set_index("INDICADOR")
    x = np.arange(len(indicadores))
    largura = 0.36

    c3 = [df.loc[i, "MEDIA_CONCEITO_3"] for i in indicadores]
    c4 = [df.loc[i, "MEDIA_CONCEITO_4"] for i in indicadores]
    ...
    ax.bar(
        ...
        c3,
        largura,
        label="UFPA — Conceito 3",
    )
    ax.bar(
        x + largura / 2,
        c4,
        largura,
        label="UFPA — Conceito 4",
    )
    ax.set_xticks(x, nomes)
    ax.set_title("Geografia — contraste interno validado da UFPA")
    ax.legend()
    _salvar(fig, path)


def sensibilidade_benchmarks_validada(
    ...
    ax.set_title("Geografia — benchmark estrutural principal por oferta")
    _salvar(fig, path)


def processo_grupos_validado(
    processo: pd.DataFrame,
    path: Path,
) -> None:
    df = processo.copy()
    df["ABS_DIF"] = pd.to_numeric(
        df["DIFERENCA_C3_C4"], errors="coerce"
    ).abs()
    df = df.nlargest(12, "ABS_DIF").sort_values("DIFERENCA_C3_C4")
    fig, ax = plt.subplots(figsize=(9, 6))
    y = np.arange(len(df))
    ax.barh(y, df["DIFERENCA_C3_C4"])
    ax.set_yticks(y, df["ITEM"])
    ax.axvline(0, linewidth=1)
    ax.set_xlabel("Média Conceito 3 − média Conceito 4")
    ax.set_title(
        "Geografia — QE_I20–QE_I66 com maiores diferenças internas"
    )
    _salvar(fig, path)


def perfil_interno_validado(
    perfil: pd.DataFrame,
    path: Path,
) -> None:
    indicadores = (
    ...
    c3 = perfil.loc[
        perfil["RECORTE_GEOGRAFIA"].eq("UFPA — Conceito 3")
    ].set_index("INDICADOR")
    c4 = perfil.loc[
        perfil["RECORTE_GEOGRAFIA"].eq("UFPA — Conceito 4")
    ].set_index("INDICADOR")
    ...
    ax.plot(
        x,
        [c3.loc[i, "MEDIA_CURSOS"] * 100 for i in indicadores],
        marker="o",
        label="UFPA — Conceito 3",
    )
    ax.plot(
        x,
        [c4.loc[i, "MEDIA_CURSOS"] * 100 for i in indicadores],
        marker="o",
        label="UFPA — Conceito 4",
    )
    ax.set_xticks(x, labels, rotation=25, ha="right")
    ax.set_ylabel("Percentual (%)")
    ax.set_title("Geografia — perfil agregado no contraste interno")
    ax.legend()
    _salvar(fig, path)


def regional_validada(
    ...
    ax.set_title("Geografia — comparação regional e nacional validada")
    ax.legend()
    _salvar(fig, path)
