...
def preparar_catalogo_portugues(extraida: Path, conceito_path: Path) -> pd.DataFrame:
    ...
    cadastro = cadastro[cadastro["CO_GRUPO"].eq(PORTUGUES.co_grupo)].drop_duplicates("CO_CURSO")
    ...
    conceitos = conceitos[conceitos["CO_GRUPO"].eq(PORTUGUES.co_grupo)].copy()
    ...
    validar_unicidade_por_curso(conceitos, nome="conceitos de Letras–Português")
    ...
    cursos = aplicar_grupos_area(cursos, PORTUGUES)
    validar_grupos(cursos)
    return cursos


def construir_auditoria_relacao_ufpa(
    cursos: pd.DataFrame,
    ofertas_informadas: list[dict],
    areas: dict[int, str],
) -> pd.DataFrame:
    ofertas = [
        oferta
        for oferta in ofertas_informadas
        if int(oferta["co_grupo"]) == PORTUGUES.co_grupo
    ]
    mestre = cursos[cursos["CO_IES"].eq(PORTUGUES.co_ies_focal)].copy()
    auditoria = comparar_relacao_informada(mestre, ofertas, areas)
    auditoria["FONTE_VALIDACAO"] = (
        "microdados2025_arq1.txt + conceito_enade_licenciaturas.xlsx"
    )
    ausente = auditoria["STATUS_VALIDACAO"].eq("Não localizado nas fontes")
    auditoria.loc[ausente, "FONTE_VALIDACAO"] = (
        "não localizado no cadastro do microdados2025_arq1.txt nem na "
        "planilha conceito_enade_licenciaturas.xlsx"
    )
    auditoria.loc[ausente, "CORRECAO_RECOMENDADA"] = (
        "Registrar a oferta informada de Soure como não localizada nas duas fontes de 2025; "
        "não atribuir CO_CURSO, inscritos, participantes, desempenho ou Conceito Enade e "
        "não incluí-la nos grupos comparativos."
    )
    return auditoria
