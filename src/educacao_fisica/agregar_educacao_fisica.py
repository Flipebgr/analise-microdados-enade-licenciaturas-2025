...
def agregar_temas_educacao_fisica(
    pasta_dados: Path,
    cursos: list[int],
) -> dict[str, pd.DataFrame]:
    """Agrega cada arquivo temático separadamente por CO_CURSO."""

    desempenho, desempenho_individual = agregar_desempenho(
        pasta_dados / "microdados2025_arq3.txt",
        cursos,
    )
    ...
    trajetoria, distribuicao_turno = agregar_trajetoria(
        pasta_dados / "microdados2025_arq2.txt",
        cursos,
    )
    ...
    processo, itens_processo, diagnostico_processo = (
        agregar_processo_formativo(
            pasta_dados / "microdados2025_arq4.txt",
            cursos,
        )
    )
