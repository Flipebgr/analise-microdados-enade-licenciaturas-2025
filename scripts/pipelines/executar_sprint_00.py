...
from src.validacao.validar_cursos_ufpa import (
    carregar_caracterizacao,
    construir_tabela_mestra,
)
from src.validacao.validar_planilha_conceito import (
    carregar_conceitos,
    localizar_ufpa,
)
from src.validacao.validar_relacao_informada import (
    comparar_relacao_informada,
)
from src.validacao.validar_relacionamentos import (
    validar_unicidade_tabela_mestra,
)
...
def main() -> int:
    ...
    salvar_csv(
        manifesto,
        ROOT / "dados_processados" / "manifesto_arquivos.csv",
        encoding_saida,
    )

    conceitos = carregar_conceitos(conceito_path)
    conceitos_ufpa = localizar_ufpa(
        conceitos,
        cfg["ufpa"]["nome_oficial"],
        cfg["ufpa"]["sigla"],
    )
    ...
    salvar_csv(
        inventario,
        ROOT / "dados_processados" / "inventario_microdados.csv",
        encoding_saida,
    )
    salvar_csv(
        ausencias,
        ROOT / "dados_processados" / "ausencias_amostra.csv",
        encoding_saida,
    )
    ...
    mestre, divergencias = construir_tabela_mestra(
        caracterizacao,
        conceitos_ufpa,
        areas,
    )
    ...
    salvar_csv(
        mestre,
        ROOT / "dados_processados" / "tabela_mestra_ufpa.csv",
        encoding_saida,
    )
    salvar_csv(
        divergencias,
        ROOT / "dados_processados" / "divergencias_ufpa.csv",
        encoding_saida,
    )
    validacao_informada = comparar_relacao_informada(
        mestre,
        cfg.get("ofertas_informadas", []),
        areas,
    )
    salvar_csv(
        validacao_informada,
        ROOT / "dados_processados" / "validacao_relacao_informada.csv",
        encoding_saida,
    )
    gerar_relatorio(
        ROOT / "relatorios" / "sprint_00_relatorio_qualidade.md",
        manifesto,
        inventario,
        mestre,
        divergencias,
    ...
    logger.info(
        "Sprint 0 concluída. Ofertas na tabela-mestra: %s",
        len(mestre),
    )
    if erros:
        for erro in erros:
