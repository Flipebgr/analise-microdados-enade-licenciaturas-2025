...
AREA_MATEMATICA = MATEMATICA.co_grupo
...
def _preparar_cadastro(cadastro: pd.DataFrame) -> pd.DataFrame:
    ...
    trabalho = trabalho[trabalho["CO_GRUPO"].eq(MATEMATICA.co_grupo)].copy()
...
def _preparar_conceitos(conceitos: pd.DataFrame) -> pd.DataFrame:
    trabalho = conceitos[conceitos["CO_GRUPO"].eq(MATEMATICA.co_grupo)].copy()
    trabalho = trabalho.loc[:, COLUNAS_CONCEITO]
    trabalho["CO_CURSO"] = normalizar_codigo(trabalho["CO_CURSO"])
    trabalho = trabalho.drop_duplicates("CO_CURSO")
    validar_unicidade_por_curso(trabalho, nome="conceitos de Matemática")
    return trabalho


def preparar_catalogo_matematica(extraida: Path, conceito_path: Path) -> pd.DataFrame:
    ...
    cursos = aplicar_grupos_area(cursos, MATEMATICA)
