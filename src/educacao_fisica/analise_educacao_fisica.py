...
def construir_comparacao_recortes(
    ...
    for recorte in recortes:
        sub = base[base["RECORTE_EDUCACAO_FISICA"].eq(recorte)]
        for indicador in indicadores:
            ...
            linhas.append(
                {
                    "RECORTE_EDUCACAO_FISICA": recorte,
                    "INDICADOR": indicador,
...
def construir_comparacao_interna_ufpa(
    base: pd.DataFrame,
) -> pd.DataFrame:
    ufpa = base[
        base["CO_IES"].eq(EDUCACAO_FISICA.co_ies_focal)
    ].copy()
    return construir_comparacao_recortes(ufpa)
