...
from src.geografia.agregar_geografia import agregar_temas_geografia, juntar_um_para_um
...
from src.geografia.comparacoes_regionais import construir_comparacoes_regionais
from src.geografia.gerar_figuras import (
    benchmarks_ufpa,
    comparacao_regional,
...
from src.geografia.preparar_catalogo import (
    preparar_catalogo_geografia,
    tabela_mestra_ufpa,
)
from src.geografia.validar_geografia import (
    validar_auditoria_fontes,
    validar_base_geografia,
)
...
def gerar_relatorio_piloto(
    ...
    linhas = [
        ...
        "## Síntese",
        "",
        f"A base analítica reúne **{len(base)} cursos** de Geografia. "
        f"Foram localizadas **{len(ufpa)} ofertas da UFPA**. "
        "Não existe oferta UFPA com Conceito Enade 1 nesta área; o Grupo A permanece vazio.",
        "",
        "As quatro ofertas da UFPA estão distribuídas entre Conceito Enade 3 e Conceito Enade 4. "
        "O contraste institucional principal é, portanto, Conceito 3 versus Conceito 4, sem tratar "
        "Conceito 3 como insuficiência e sem atribuir causalidade ao conceito.",
        "",
        "A unidade principal é `CO_CURSO`. Cada arquivo temático é tratado e agregado separadamente "
        "antes das junções one-to-one.",
        "",
        "## Ofertas da UFPA",
        "",
        ufpa[colunas].sort_values("ROTULO_OFERTA").to_markdown(
            index=False,
            floatfmt=".3f",
        ),
        "",
        ...
        "",
        "As quatro ofertas foram localizadas tanto no cadastro dos microdados quanto na planilha "
        "de Conceito Enade 2025.",
        "",
...
def main() -> int:
    garantir_pastas()
    cfg = carregar_config()
    logger = configurar_logger(ROOT / "logs" / "sprint_19_geografia.log")
    ...
    logger.info("Preparando catálogo nacional de Geografia")
    cursos, auditoria = preparar_catalogo_geografia(extraida, conceito_path)
    validar_auditoria_fontes(auditoria)

    codigos = cursos["CO_CURSO"].astype(int).tolist()
    logger.info(
        "Agregando desempenho, perfil, trajetória, processo formativo e recomendação"
    )
    temas = agregar_temas_geografia(pasta_dados, codigos)
    ...
    out = ROOT / "dados_processados" / "geografia"
    produtos = {
        "cursos_geografia.csv": cursos,
    ...
    benchmarks_ufpa(
        benchmarks_resumo,
        figdir / "11_benchmarks_ofertas_ufpa.png",
    )
    recomendacao(base, figdir / "12_recomendacao.png")
    contraste_interno_ufpa(
        comparacao_interna,
        figdir / "13_contraste_interno_ufpa.png",
    )
    ...
    gerar_relatorio_piloto(
        ...
        comparacao_recortes,
        comparacao_interna,
        ROOT / "relatorios" / "sprint_19_piloto_geografia.md",
    )

    logger.info(
        "Sprint 19 concluída: %s cursos de Geografia, %s ofertas da UFPA e 13 figuras",
        len(base),
        int(base["CO_IES"].eq(GEOGRAFIA.co_ies_focal).sum()),
    )
    return 0
