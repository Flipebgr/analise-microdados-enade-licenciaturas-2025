...
def carregar_processo_individual(
    ...
    itens = sorted(
        {item for valores in DIMENSOES_TEORICAS.values() for item in valores}
    )
...
def catalogo_itens_processo() -> pd.DataFrame:
    ...
    # Itens específicos de EaD não entram nas dimensões comuns.
...
def diagnosticar_dimensoes(
    ...
    linhas: list[dict[str, object]] = []
    recortes = {
        "Brasil — Música": individual,
        "UFPA — Música": individual[individual["CO_CURSO"].isin(cursos_ufpa)],
    }
            ...
            linhas.append(
                {
                    ...
                    "ITENS_INVERTIDOS": "",
                    "DECISAO": (
                        "Dimensão teórica exploratória; usar apenas como "
                        "síntese descritiva e com ressalva de unidimensionalidade."
                    ),
                }
            )
...
def agregar_dimensoes_por_curso(
    ...
    for dimensao, itens in DIMENSOES_TEORICAS.items():
        ...
        escore = x.mean(axis=1, skipna=True).where(
            x.notna().sum(axis=1).ge(minimo)
        )
        tmp = pd.DataFrame(
            {
...
def sensibilidade_benchmarks(
    ...
    alvos = base[base["CO_IES"].eq(MUSICA.co_ies_focal)].copy()
    ...
    for _, alvo in alvos.iterrows():
        ...
        for criterio, (inferior, superior) in CRITERIOS_SENSIBILIDADE.items():
            ...
            if not sel.empty:
                ...
                tmp = sel[[c for c in cols if c in sel.columns]].copy()
...
def _outliers_tukey(serie: pd.Series) -> pd.Series:
    ...
    inferior = q1 - 1.5 * iqr
    superior = q3 + 1.5 * iqr
    return s.lt(inferior) | s.gt(superior)
...
def associacoes_ecologicas(
    ...
    for xcol, ycol in pares:
        ...
        cols = ["CO_CURSO", xcol, ycol, "PARTICIPANTES_NUM"]
        tmp = base[cols].copy()
        ...
        ox = _outliers_tukey(tmp[xcol])
        oy = _outliers_tukey(tmp[ycol])
        diagnostico.append(
            {
                "X": xcol,
                "Y": ycol,
                "N_CURSOS": len(tmp),
                "X_MIN": tmp[xcol].min(),
                "X_MEDIANA": tmp[xcol].median(),
                "X_MAX": tmp[xcol].max(),
                "Y_MIN": tmp[ycol].min(),
                "Y_MEDIANA": tmp[ycol].median(),
                "Y_MAX": tmp[ycol].max(),
                "N_OUTLIERS_X": int(ox.sum()),
                "N_OUTLIERS_Y": int(oy.sum()),
                "N_OUTLIERS_QUALQUER": int((ox | oy).sum()),
                "MEDIANA_PARTICIPANTES": pd.to_numeric(
    ...
    return pd.DataFrame(linhas), pd.DataFrame(diagnostico)


def resumo_qualidade(
    base: pd.DataFrame,
    catalogo_itens: pd.DataFrame,
    diagnostico: pd.DataFrame,
    benchmark: pd.DataFrame,
) -> pd.DataFrame:
    ufpa = base[base["CO_IES"].eq(MUSICA.co_ies_focal)]
    checks = [
        ("cursos_nacionais", len(base) == 107, len(base)),
        ("ofertas_ufpa", len(ufpa) == 1, len(ufpa)),
        (
            "ufpa_conceito_1",
            set(
                pd.to_numeric(
        ...
        (
            "grupos_b_c_vazios",
            not base["GRUPO_CODIGO"].isin(["B", "C"]).any(),
            int(base["GRUPO_CODIGO"].isin(["B", "C"]).sum()),
        ),
        (
            "itens_processo_20_66",
            catalogo_itens["ITEM"].nunique() == 47,
            catalogo_itens["ITEM"].nunique(),
        ),
        (
            "sem_itens_invertidos",
            len(ITENS_INVERTIDOS) == 0,
            len(ITENS_INVERTIDOS),
        ),
        (
            "dimensoes_diagnosticadas",
            diagnostico["DIMENSAO"].nunique() == 8,
            diagnostico["DIMENSAO"].nunique(),
        ),
        (
            "benchmark_oferta_ufpa",
            benchmark["CO_CURSO_ALVO"].nunique() == 1,
            benchmark["CO_CURSO_ALVO"].nunique(),
        ),
    ]
    return pd.DataFrame(
        checks,
